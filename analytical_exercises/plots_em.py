...
# -------------------------------------------------
# 1. EM parameter values (SCALE parameterization)
# -------------------------------------------------
# Professor's model:
#   p(x1 | θ1) = (1/θ1) * exp(-x1 / θ1),  x1 ≥ 0
#   p(x2 | θ2) = (1/θ2),                  0 ≤ x2 ≤ θ2
#
# θ(0) = (θ1(0), θ2(0)) = (2, 3)
theta0_1 = 2.0   # θ1(0): scale (mean) before EM
theta0_2 = 3.0   # θ2(0): upper bound of uniform before EM

# θ(1) = (θ1(1), θ2(1)) = (7/3, 5)
theta1_1 = 7.0 / 3.0  # θ1(1): scale (mean) after EM
theta1_2 = 5.0        # θ2(1): upper bound of uniform after EM
...
# x2 ranges:
#   - Before EM: [0, θ2(0)] = [0, 3]
#   - After EM : [0, θ2(1)] = [0, 5]
x2_before = np.linspace(0, theta0_2, 150)
x2_after  = np.linspace(0, theta1_2, 150)

# Create 2D grids (mesh) of x1 and x2 values for surface plots
...
# ---------- Before EM: θ(0) = (2, 3) ----------
# p(x1 | θ1(0)) = (1 / θ1(0)) * exp(-x1 / θ1(0)), x1 ≥ 0
p1_before = (1.0 / theta0_1) * np.exp(-X1_before / theta0_1)

# p(x2 | θ2(0)) = 1 / θ2(0), 0 ≤ x2 ≤ θ2(0)
# (we already restrict x2_before to [0, θ2(0)])
p2_before = (1.0 / theta0_2) * np.ones_like(X2_before)

# Joint density (properly normalized on [0, ∞) × [0, θ2(0)])
p_before = p1_before * p2_before

# ---------- After EM: θ(1) = (7/3, 5) ----------
# p(x1 | θ1(1)) = (1 / θ1(1)) * exp(-x1 / θ1(1)), x1 ≥ 0
p1_after = (1.0 / theta1_1) * np.exp(-X1_after / theta1_1)

# p(x2 | θ2(1)) = 1 / θ2(1), 0 ≤ x2 ≤ θ2(1)
p2_after = (1.0 / theta1_2) * np.ones_like(X2_after)

# Joint density (properly normalized on [0, ∞) × [0, θ2(1)])
...
ax2.set_title('p(x1, x2) at θ(1) = (7/3, 5)')
ax2.set_xlabel('x1')
ax2.set_ylabel('x2')
